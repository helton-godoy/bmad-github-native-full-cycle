...
Examples:
  ...
  async start() {
    ...
    console.log(
      `⏱️  Duration: ${this.args.duration ? `${this.args.duration}ms` : 'unlimited'}`
    );
    console.log(
      `📊 Limits: ${this.args.maxProcesses} processes, ${this.args.maxMemory}MB memory, ${this.args.maxCpu}% CPU`
    );
    ...
    if (this.args.duration) {
      setTimeout(() => {
        console.log(
          `⏰ Duration limit reached (${this.args.duration}ms), stopping...`
        );
        this.stop();
      }, this.args.duration);
    }
  ...
  setupEventListeners() {
    this.monitor.on('process_created', (processData) => {
      console.log(
        `➕ Process created: PID ${processData.pid} (${processData.type}) - ${processData.command}`
      );
    });

    this.monitor.on('process_destroyed', (processData) => {
      const lifetime = processData.lifetime
        ? `${(processData.lifetime / 1000).toFixed(2)}s`
        : 'unknown';
      console.log(
        `➖ Process destroyed: PID ${processData.pid} (${processData.type}) - lifetime: ${lifetime}`
      );
    });
    ...
    setInterval(() => {
      ...
      console.log(
        `📊 Status: ${stats.activeProcesses} active, ${stats.totalProcessesCreated} total created, ${stats.alerts.length} alerts`
      );
  ...
  displaySummary(report) {
    ...
    console.log(
      `⏱️  Total Duration: ${(report.summary.monitoringDuration / 1000).toFixed(2)}s`
    );
    console.log(
      `🔢 Processes Created: ${report.summary.totalProcessesCreated}`
    );
    console.log(
      `🔢 Processes Destroyed: ${report.summary.totalProcessesDestroyed}`
    );
    ...
    console.log('\n⏱️  Process Lifetimes:');
    console.log(
      `  Average: ${(report.lifetimeStatistics.average / 1000).toFixed(2)}s`
    );
    console.log(
      `  Median: ${(report.lifetimeStatistics.median / 1000).toFixed(2)}s`
    );
