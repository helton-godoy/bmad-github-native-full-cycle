...
class ProcessMonitor extends EventEmitter {
  ...
  async scanExistingProcesses() {
    try {
      ...
      this.logger.info(
        `Found ${relevantProcesses.length} existing relevant processes`
      );
    } catch (error) {
  ...
  async getSystemProcesses() {
    return new Promise((resolve, reject) => {
      exec(
        'ps -eo pid,ppid,command,etime,pcpu,pmem --no-headers',
  ...
  handleProcessCreation(processInfo) {
    ...
    this.statistics.peakProcesses = Math.max(
      this.statistics.peakProcesses,
      this.statistics.activeProcesses
    );

    // Update process type statistics
    const processType = processData.type;
    this.statistics.processTypes[processType] =
      (this.statistics.processTypes[processType] || 0) + 1;
  ...
  handleProcessDestruction(pid, exitCode = null) {
    ...
    this.statistics.averageLifetime =
      completedProcesses > 0 ? totalLifetime / completedProcesses : 0;
  ...
  async monitorExistingProcesses() {
    ...
    try {
      ...
      for (const systemProc of systemProcesses) {
        if (
          !this.processes.has(systemProc.pid) &&
          this.isRelevantProcess(systemProc)
        ) {
          this.handleProcessCreation({
            pid: systemProc.pid,
  ...
  classifyProcess(command, args = []) {
    ...
    if (fullCommand.includes('node') && fullCommand.includes('test'))
      return 'node_test';
  ...
  async getSystemResources() {
    try {
      ...
      const memAvailable =
        parseInt(memInfo.match(/MemAvailable:\s+(\d+)/)[1]) * 1024;
  ...
  async killAllProcesses(signal = 'SIGTERM') {
    ...
    for (const processData of activeProcesses) {
      try {
        ...
        this.logger.info(
          `Killed process ${processData.pid} with signal ${signal}`
        );
      } catch (error) {
        results.push({
          pid: processData.pid,
        ...
        this.logger.error(
          `Failed to kill process ${processData.pid}: ${error.message}`
        );
      }
    }
