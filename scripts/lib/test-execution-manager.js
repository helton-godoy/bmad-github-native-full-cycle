...
class TestExecutionManager {
  ...
  async acquireLock(processId = 'unknown') {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (fs.existsSync(this.lockFile)) {
          ...
          if (lockAge > this.lockTimeout) {
            ...
            if (!this.isProcessRunning(existingLock.pid)) {
              this.logger.warn(
                `Removing lock from dead process (PID: ${existingLock.pid})`
              );
              fs.unlinkSync(this.lockFile);
            } else {
              this.logger.info(
                `Test execution locked by process ${existingLock.processId} (PID: ${existingLock.pid}), waiting...`
              );
        ...
        if (error.code === 'EEXIST') {
          ...
          this.logger.info(
            `Lock acquired by another process, attempt ${attempt}/${this.maxRetries}`
          );
          await this.sleep(this.retryDelay * attempt);
          continue;
        } else {
    ...
    throw new Error(
      `Failed to acquire test execution lock after ${this.maxRetries} attempts`
    );
  }
    ...
    try {
      if (fs.existsSync(this.lockFile)) {
        ...
        if (lockData.pid === process.pid) {
          ...
          this.logger.warn(
            `Cannot release lock owned by PID ${lockData.pid} (we are ${process.pid})`
          );
        }
      }
    } catch (error) {
  ...
  async executeTestsInBatches(testFiles, options = {}) {
    ...
    this.logger.info(
      `Executing ${testFiles.length} test files in batches of ${batchSize}`
    );
    ...
    for (let i = 0; i < testFiles.length; i += batchSize) {
      ...
      this.logger.info(
        `Executing batch ${batchId}/${totalBatches} (${batch.length} files)`
      );
  ...
  hasEnoughResources(thresholds = {}) {
    ...
    if (!memoryOk) {
      this.logger.warn(
        `High memory usage: ${resources.memory.usagePercent.toFixed(1)}%`
      );
    }
