...
class GitStateManager {
  ...
  init() {
    try {
      ...
      const emptyTreeHash = execSync('git hash-object -t tree /dev/null')
        .toString()
        .trim();
      const commitHash = execSync(
        `echo "Initial State" | git commit-tree ${emptyTreeHash}`
      )
        .toString()
        .trim();
  ...
  read(filePath) {
    try {
      ...
      return execSync(`git show ${this.branchName}:${filePath}`, {
        encoding: 'utf-8',
  ...
  write(filePath, content) {
    ...
    try {
      ...
      const treeHash = execSync('git write-tree', { env, encoding: 'utf-8' })
        .toString()
        .trim();
      ...
      let parentArg = '';
      try {
        const parentHash = execSync(`git rev-parse ${this.branchName}`, {
          encoding: 'utf-8',
        })
          .toString()
          .trim();
      ...
      const commitHash = execSync(
        `echo "${commitMsg}" | git commit-tree ${treeHash} ${parentArg}`,
        { encoding: 'utf-8' }
      )
        .toString()
        .trim();
  ...
  list() {
    try {
      const output = execSync(`git ls-tree -r --name-only ${this.branchName}`, {
        encoding: 'utf-8',
      });
      return output.split('\n').filter(Boolean);
    } catch (e) {
