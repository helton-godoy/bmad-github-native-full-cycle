...
class JestProcessMonitor {
  constructor(options = {}) {
    this.options = {
      ...
      reportPath:
        options.reportPath || '.github/reports/process-monitor-report.json',
      ...options,
    };
  ...
  hookChildProcess() {
    ...
    childProcess.spawn = (command, args = [], options = {}) => {
      const child = this.originalSpawn.call(
        childProcess,
        command,
        args,
        options
    ...
    childProcess.exec = (command, options, callback) => {
      ...
      const child = this.originalExec.call(
        childProcess,
        command,
        options,
        callback
  ...
  classifyJestWorker(processData) {
    const fullCommand =
      `${processData.command} ${processData.args.join(' ')}`.toLowerCase();
  ...
  async saveReport(report) {
    ...
    try {
      ...
      this.logger.info(
        `Process monitoring report saved to ${this.options.reportPath}`
      );
    } catch (error) {
  ...
  enhanceReportWithJestData(report) {
    const jestProcesses = report.detailedProcesses.filter(
      (p) => p.jestWorker || this.isJestWorker(p)
    );
    ...
    for (const process of jestProcesses) {
      ...
      jestStats.jestWorkerTypes[workerType] =
        (jestStats.jestWorkerTypes[workerType] || 0) + 1;
    ...
    if (jestProcessesWithLifetime > 0) {
      jestStats.averageJestProcessLifetime =
        totalJestLifetime / jestProcessesWithLifetime;
    }
  ...
  getJestStatistics() {
    ...
    return {
      totalJestProcesses: jestProcesses.length,
      activeJestProcesses: jestProcesses.filter((p) => p.status === 'running')
        .length,
      jestMemoryUsage: jestProcesses.reduce(
        (sum, p) => sum + (p.memoryUsage || 0),
        0
      ),
      jestCpuUsage: jestProcesses.reduce(
        (sum, p) => sum + (p.cpuUsage || 0),
        0
      ),
      jestProcesses: jestProcesses,
    };
  ...
  async killAllJestProcesses() {
    const jestProcesses = Array.from(this.monitor.processes.values()).filter(
      (p) => this.isJestWorker(p) && p.status === 'running'
    );
