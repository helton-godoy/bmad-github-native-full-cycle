...
class ConfigLoader {
  ...
  applyEnvironmentOverrides(config) {
    const envMappings = {
      ...
      BMAD_MAX_RECOVERY_ATTEMPTS:
        'components.errorRecoveryManager.maxRecoveryAttempts',
      BMAD_RECOVERY_PERSONA_ENABLED:
        'components.errorRecoveryManager.enableRecoveryPersona',
      BMAD_EXPONENTIAL_BACKOFF_INITIAL_DELAY:
        'components.errorRecoveryManager.backoffConfig.initialDelay',
      BMAD_EXPONENTIAL_BACKOFF_MAX_DELAY:
        'components.errorRecoveryManager.backoffConfig.maxDelay',
      BMAD_EXPONENTIAL_BACKOFF_MULTIPLIER:
        'components.errorRecoveryManager.backoffConfig.multiplier',
      ...
      BMAD_LOOP_DETECTION_MAX_TRANSITIONS:
        'components.loopDetector.maxTransitions',
      ...
      BMAD_REQUIRE_CONTEXT_UPDATE:
        'components.enhancedGatekeeper.requireContextUpdate',
      ...
      BMAD_COMMIT_STAGING_VALIDATION:
        'components.commitHandler.validateStaging',
  ...
  validate() {
    ...
    const numericValidations = [
      ...
      {
        path: 'components.stateCacheManager.lockTimeout',
        min: 1000,
        max: 300000,
      },
    ...
    for (const validation of numericValidations) {
      ...
      if (
        ...
        errors.push(
          `Invalid value for ${validation.path}: must be between ${validation.min} and ${validation.max}`
        );
      }
    }
