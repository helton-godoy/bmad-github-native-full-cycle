...
class EnhancedGatekeeper {
  ...
  async validateCommitMessage(commitMsg, result) {
    ...
    if (bmadPattern.test(commitMsg) || conventionalPattern.test(commitMsg)) {
      ...
      result.errors.push({
        ...
        remediation:
          'Use format: [PERSONA] [STEP-ID] Description or conventional commits format',
      });
    }
  }
  ...
  async validateContextUpdate(result) {
    ...
    try {
      ...
      const hasCodeChanges = staged.match(
        /\.(js|ts|jsx|tsx|py|rb|go|rs|java|c|cpp|h|hpp|css|scss|html|vue|svelte)$/m
      );
    ...
    switch (error.type) {
      case 'COMMIT_FORMAT_ERROR':
        remediation.immediate.push(
          'Fix commit message format to match [PERSONA] [STEP-ID] Description'
        );
        remediation.immediate.push(
          'Use conventional commits format as alternative'
        );
        remediation.longTerm.push(
          'Set up commit message templates in .gitmessage'
        );
        remediation.longTerm.push('Configure git hooks for commit validation');
        remediation.automated.push(
      ...
      case 'CONTEXT_UPDATE_ERROR':
        remediation.immediate.push(
          'Update activeContext.md to reflect current changes'
        );
        ...
        remediation.automated.push(
          'Set up pre-commit hook to remind about context updates'
        );
        break;
      ...
      case 'VALIDATION_ERROR':
        remediation.immediate.push(
          'Review validation error details and fix accordingly'
        );
        remediation.immediate.push(
  ...
  logSuccessValidation(result) {
    ...
    console.log(
      `${colors.green}✅ BMAD Enhanced Gatekeeper - VALIDATION PASSED${colors.reset}`
    );
  ...
  reportDetailedError(result) {
    ...
    console.log(
      `${colors.red}❌ BMAD Enhanced Gatekeeper - VALIDATION FAILED${colors.reset}`
    );
    console.log(`Timestamp: ${result.timestamp}`);
    console.log(`Gate Status: ${colors.red}${result.gate}${colors.reset}`);
    console.log(
      `Severity: ${this.getStatusColor(errorReport.severity)}${errorReport.severity}${colors.reset}`
    );
    ...
    console.log(
      `  • Workflow Blocked: ${errorReport.impact.workflowBlocked ? 'Yes' : 'No'}`
    );
    console.log(
      `  • Affected Components: ${errorReport.impact.affectedComponents.join(', ')}`
    );
    console.log(
      `  • Estimated Fix Time: ${errorReport.impact.estimatedFixTime}`
    );
  ...
  enableDevelopmentMode(bypass = true, reason = 'Development testing') {
    ...
    this.logger.info(
      `Development bypass ${bypass ? 'enabled' : 'disabled'}: ${reason}`
    );
  ...
  checkBypass(validationType) {
    if (this.config.bypassEnabled && this.config.developmentMode) {
      ...
      this.logger.warn(
        `Bypassing ${validationType} validation (development mode)`
      );
    ...
    switch (hookType) {
      case 'pre-commit':
        ...
        if (errors.some((e) => e.type === 'TEST_FAILURE')) {
          recommendations.push(
            'Run npm test to see detailed test failure information'
          );
        }
        if (errors.some((e) => e.type === 'CONTEXT_UPDATE_ERROR')) {
          recommendations.push(
            'Update activeContext.md to document your changes'
          );
        }
        break;
      ...
      case 'pre-push':
        if (errors.some((e) => e.type === 'COVERAGE_ERROR')) {
          ...
          recommendations.push(
            'Run npm run test:coverage to see coverage details'
          );
        }
      ...
      case 'post-merge':
        if (errors.some((e) => e.type === 'WORKFLOW_ERROR')) {
          recommendations.push(
            'Check BMAD workflow logs for detailed error information'
          );
