...
class PerformanceMonitor {
  ...
  detectOptimizationOpportunity(execution) {
    ...
    if (recentExecutions.length >= 3) {
      const avgDuration =
        recentExecutions.reduce((sum, exec) => sum + exec.duration, 0) /
  ...
  generateOptimizationRecommendations(execution, avgDuration) {
    ...
    if (execution.hookType === 'pre-commit') {
      if (avgDuration > 10000) {
        recommendations.push(
          'Consider running only fast tests in pre-commit hook'
        );
    ...
    if (avgDuration > this.config.performanceThreshold * 2) {
      recommendations.push(
        'Enable development mode bypass for faster local workflow'
      );
    ...
    if (
      ...
      avgDuration > this.config.optimizationThreshold
    ) {
      recommendations.push('Review hook execution performance');
      recommendations.push(
        'Consider optimization strategies for this hook type'
      );
    }
  ...
  getMetrics() {
    ...
    const thresholdMetExecutions = this.executions.filter(
      (exec) => exec.performanceThresholdMet
    ).length;
    const optimizableExecutions = this.executions.filter(
      (exec) => exec.optimizable
    ).length;
    ...
    const byHookType = {};
    const hookTypes = [
      ...new Set(this.executions.map((exec) => exec.hookType)),
    ];

    hookTypes.forEach((hookType) => {
      const hookExecutions = this.executions.filter(
        (exec) => exec.hookType === hookType
      );
      const hookDuration = hookExecutions.reduce(
        (sum, exec) => sum + exec.duration,
        0
      );
    ...
    return {
      ...
      performanceThresholdMetRate:
        thresholdMetExecutions / this.executions.length,
