...
class ContextManager {
  ...
  withLock(lockName, operation) {
    ...
    if (!acquired) {
      throw new Error(
        `Failed to acquire lock for ${lockName} after ${this.retryOptions.retries} attempts`
      );
    }
  ...
  recordFailure(_component = 'workflow') {
    ...
    if (state.failures >= 3) {
      ...
      const firstFailure = state.firstFailure
        ? new Date(state.firstFailure)
        : new Date();

      if (firstFailure > oneHourAgo) {
        state.isOpen = true;
        console.error(
          '🔥 CIRCUIT BREAKER OPENED: Too many failures in short period.'
        );
      } else {
  ...
  resetFailure(_component = 'workflow') {
    ...
    this.write(
      cbFile,
      JSON.stringify(
        { failures: 0, lastFailure: null, isOpen: false, firstFailure: null },
        null,
        2
      )
    );
