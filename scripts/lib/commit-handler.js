...
class CommitHandler {
  ...
  async executeCommit(message, persona, stepId) {
    try {
      ...
      if (
        this.validateFormat &&
        !this._validateMessageFormat(formattedMessage)
      ) {
  ...
  validateMessageFormat(message) {
    ...
    try {
      ...
      if (!match) {
        result.errors.push(
          'Message does not match required pattern: [PERSONA] [STEP-ID] Description'
        );
        return result;
      }
        ...
        if (match) {
          ...
          if (pattern === patterns[0]) {
            ...
            result.corrections.push(
              'Added brackets around persona and step ID'
            );
            ...
            correctedMessage = `[${persona}] [STEP-${stepId}] ${description}`;
            result.corrections.push(
              'Generated step ID and added proper formatting'
            );
          }
    ...
    if (!/^[0-9]{3}$/.test(stepId)) {
      if (/^[0-9]+$/.test(stepId)) {
        if (stepId.length < 3) {
          ...
          result.warnings.push(
            'Step ID should be 3 digits (consider shortening)'
          );
        }
      } else {
        result.valid = false;
  ...
  _validateDescription(description) {
    ...
    if (
      trimmedDescription.toLowerCase().includes('fix') &&
      trimmedDescription.length < 20
    ) {
      result.warnings.push(
        'Generic "fix" description - consider being more specific'
      );
    }

    if (
      trimmedDescription.toLowerCase().includes('update') &&
      trimmedDescription.length < 25
    ) {
      result.warnings.push(
        'Generic "update" description - consider being more specific'
      );
    }
  ...
  async validateCommit(commitHash) {
    try {
      ...
      if (
        this.validateFormat &&
        !this._validateMessageFormat(commitInfo.message)
      ) {
        this.logger.warn(
          `Commit ${commitHash} has invalid message format: ${commitInfo.message}`
        );
      }
      ...
      this.logger.error(
        `Commit verification failed for ${commitHash}: ${error.message}`
      );
  ...
  async rollbackCommit(commitHash) {
    try {
      ...
      if (headHash !== commitHash) {
        throw new Error(
          `Cannot rollback commit ${commitHash} - it is not the latest commit (HEAD: ${headHash})`
        );
      }
      ...
      this.logger.error(
        `Rollback failed for commit ${commitHash}: ${error.message}`
      );
      throw error;
    }
  }
  ...
  async _getCommitInfo(commitHash) {
    try {
      ...
      const output = execSync(
        `git show --format="%H|%an|%ae|%at|%s" --name-only ${commitHash}`,
        {
          encoding: 'utf-8',
        }
      );
      ...
      throw new Error(
        `Failed to get commit info for ${commitHash}: ${error.message}`
      );
    }
  }
  ...
  async _isCommitInCurrentBranch(commitHash) {
    try {
      ...
      execSync(`git merge-base --is-ancestor ${commitHash} HEAD`, {
        stdio: 'pipe',
      });
      return true;
    } catch (error) {
