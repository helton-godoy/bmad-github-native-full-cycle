...
if (
  commitMsg &&
  !/^(feat|fix|docs|style|refactor|perf|test|build|ci|chore|revert)(\([\w-]+\))?: .+/.test(
    commitMsg
  )
) {
  console.error(
    `${colors.red}❌ ERRO: Mensagem de commit inválida.${colors.reset}`
  );
  console.error(
    '   Esperado: "type(scope): description" (ex: feat(auth): add login)'
  );
  process.exit(1);
}
...
if (config.requireContextUpdate) {
  try {
    ...
    if (hasCodeChanges && !hasContextUpdate) {
      ...
      // eslint-disable-next-line quotes
      console.error(
        "   Detectadas alterações de código sem atualização do 'activeContext.md'."
      );
      // eslint-disable-next-line quotes
      console.error(
        '   Regra: O Agente DEVE atualizar a memória do projeto antes de finalizar.'
      );
      process.exit(1);
    }
  } catch {
...
console.log(
  `${colors.yellow}🧪 A verificar integridade (testes)...${colors.reset}`
);
...
if (process.env.BMAD_SKIP_TESTS === 'true') {
  console.log(
    `${colors.yellow}⚠️  BMAD_SKIP_TESTS ativado. Pulando execução de testes.${colors.reset}`
  );
  ...
  try {
    ...
    if (pkg.scripts && pkg.scripts.test) {
      ...
      console.log(
        `${colors.yellow}⚠️  Nenhum script de teste encontrado. Pulando testes.${colors.reset}`
      );
    ...
    console.error(
      `${colors.red}❌ Testes falharam. Commit abortado.${colors.reset}`
    );
