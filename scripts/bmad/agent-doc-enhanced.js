...
class EnhancedAgentDoc {
  ...
  extractTags() {
    ...
    this.metrics.tagsExtracted = Object.values(this.semanticTags).reduce(
      (sum, tags) => sum + tags.length,
      0
    );
    console.log(
      `📊 Extracted ${this.metrics.tagsExtracted} tags from ${this.metrics.filesProcessed} files`
    );
  }
  ...
  generateDocumentation() {
    ...
    console.log(
      `📚 Generated ${this.metrics.documentationGenerated} documentation sections`
    );
  }
...
`;
  ...
  generateMermaidDiagram() {
    ...
    connections.forEach((tag) => {
      ...
      const connectMatch = content.match(
        /connects?\s+(?:to|with)?\s+([a-zA-Z0-9\s]+)/i
      );
  ...
  extractProjectDescription(contexts) {
    const descriptions = contexts.filter(
      (tag) =>
  ...
  extractDevelopmentApproach(contexts) {
    const approaches = contexts.filter(
      (tag) =>
  ...
  formatInvariants(invariants) {
    return invariants.length > 0
      ? invariants
          .map(
            (tag) =>
              `- **${tag.content}** (\`${path.basename(tag.file)}\`:${tag.line})`
          )
          .join('\n')
  ...
  formatPersonas(personas) {
    return personas.length > 0
      ? personas
          .map((tag) => `- **${tag.content}** (\`${path.basename(tag.file)}\`)`)
          .join('\n')
      : 'No personas defined.';
  }
  ...
  extractOptimizationStrategies(performanceTags) {
    const strategies = performanceTags.filter(
      (tag) =>
