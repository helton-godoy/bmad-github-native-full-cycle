...
function generateMarkdown(mapData) {
  ...
  for (const [file, tags] of Object.entries(mapData)) {
    ...
    if (invariants.length > 0) {
      md += '### 🛡️ Invariants (DO NOT BREAK)\n';
      invariants.forEach((t) => (md += `- 🔴 **${t.content}**\n`));
    }

    if (connections.length > 0) {
      md += '### 🔗 Connections\n';
      connections.forEach((t) => (md += `- ➡️ ${t.content}\n`));
    }
    md += '\n---\n\n';
  }
  return md;
...
async function qdrantRequest(endpoint, method = 'GET', body = null) {
  try {
    ...
    console.error(`❌ Qdrant Error (${endpoint}):`, e.message);
    return null;
  }
}
...
async function syncToQdrant(mapData) {
  ...
  if (points.length > 0) {
    ...
    if (result?.status === 'ok') {
      console.log('✅ Qdrant Sync Successful!');
    } else {
      console.error('❌ Qdrant Sync Failed:', result);
    }
  }
}
