...
async function runDemo() {
  ...
  monitor.on('process_created', (processData) => {
    console.log(
      `✅ Process created: PID ${processData.pid} (${processData.type})`
    );
  });
  ...
  monitor.on('process_destroyed', (processData) => {
    const lifetime = processData.lifetime
      ...
      : 'unknown';
    console.log(
      `❌ Process destroyed: PID ${processData.pid} - lifetime: ${lifetime}`
    );
  });
  ...
  try {
    ...
    console.log(
      `Total Processes Created: ${report.summary.totalProcessesCreated}`
    );
    ...
    if (report.alerts.length > 0) {
      ...
      report.alerts.forEach((alert, index) => {
        console.log(
          `  ${index + 1}. ${alert.type}: ${JSON.stringify(alert.data)}`
        );
      });
    }
