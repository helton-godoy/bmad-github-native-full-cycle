...
async function testHooks() {
  ...
  try {
    ...
    const preCommitResult = await orchestrator.executePreCommit([
      'test.js',
      'example.md',
    ]);
    ...
    const commitMsgResult = await orchestrator.executeCommitMsg(
      '[DEVELOPER] [STEP-001] Test commit message'
    );
