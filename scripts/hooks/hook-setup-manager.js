...
class HookSetupManager {
    ...
    try {
      ...
      if (existingHooks.hasHooks && !this.options.forceReinstall) {
        ...
        result.warnings.push(
          'Existing hooks detected - use forceReinstall option to overwrite'
        );
        ...
        if (!validation.allValid) {
          result.warnings.push(
            'Some existing hooks are not valid - consider reinstalling'
          );
        }

        result.success = validation.allValid;
        result.hooksCreated = existingHooks.hooks
          .filter((h) => h.exists)
          .map((h) => h.name);
        return result;
      }
      ...
      for (const hookName of this.requiredHooks) {
        try {
          ...
          result.errors.push(
            `Failed to create ${hookName} hook: ${error.message}`
          );
          this.logger.error(
            `Failed to create ${hookName} hook: ${error.message}`
          );
  ...
  detectExistingHooks() {
    ...
    try {
      ...
      for (const hookName of this.requiredHooks) {
        ...
        if (exists) {
          try {
            ...
            this.logger.warn(
              `Could not get stats for ${hookName}: ${error.message}`
            );
          }
        }
      ...
      this.logger.info(
        `Found ${result.hooks.filter((h) => h.exists).length} existing hooks`
      );
      return result;
    } catch (error) {
  ...
  async updateHooks() {
    ...
    try {
      ...
      if (!existingHooks.hasHooks) {
        result.warnings.push(
          'No existing hooks found - use installHooks() instead'
        );
      ...
      for (const hookInfo of existingHooks.hooks) {
        if (hookInfo.exists) {
          try {
            ...
            result.errors.push(
              `Failed to update ${hookInfo.name}: ${error.message}`
            );
            this.logger.error(
              `Failed to update ${hookInfo.name}: ${error.message}`
            );
    ...
    try {
      ...
      for (const hookName of this.requiredHooks) {
        ...
        result.checks.push({
          ...
          passed: exists,
          message: exists
            ? `${hookName} hook exists`
            : `${hookName} hook not found`,
        });
        ...
        try {
          ...
          result.warnings.push(
            `Could not check ${hookName} permissions: ${error.message}`
          );
        }
        ...
        try {
          ...
          result.checks.push({
            ...
            message: contentValid
              ? `${hookName} content is valid`
              : `${hookName} content may be invalid`,
          });

          if (!contentValid) {
            result.warnings.push(
              `${hookName} content may be invalid or outdated`
            );
          }
        } catch (error) {
          result.checks.push({
          ...
          result.warnings.push(
            `Could not read ${hookName} content: ${error.message}`
          );
        }
      }
      ...
      result.checks.push({
        ...
        message: orchestratorExists
          ? 'Hook Orchestrator found'
          : 'Hook Orchestrator not found',
      });

      if (!orchestratorExists) {
        result.allValid = false;
        result.errors.push(
          'Hook Orchestrator not found - hooks will not function'
        );
      }

      this.logger.info(
        `Validation completed: ${result.allValid ? 'PASSED' : 'FAILED'}`
      );
      return result;
    } catch (error) {
  ...
  generateConfigReport() {
    ...
    try {
      ...
      if (!report.validation.allValid) {
        report.recommendations.push(
          'Fix validation errors before using git hooks'
        );

        if (report.validation.errors.length > 0) {
          report.recommendations.push(
            ...report.validation.errors.map((err) => `Fix: ${err}`)
          );
        }
      }
      ...
      if (outdatedHooks.length > 0) {
        report.recommendations.push(
          `Update outdated hooks: ${outdatedHooks.map((h) => h.name).join(', ')}`
        );
      }
  ...
  createHookScript(hookName) {
    ...
    try {
      ...
      this.logger.warn(
        `Could not set executable permissions for ${hookName}: ${error.message}`
      );
    }
