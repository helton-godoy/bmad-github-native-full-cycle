...
class HookErrorHandler {
  ...
  classifyError(error, hookType) {
    ...
    const blockingPatterns = [
      ...
      {
        pattern: /security.*vulnerabilit/i,
        category: 'SECURITY_VULNERABILITY',
        severity: 'blocking',
      },
      {
        pattern: /invalid.*commit.*message/i,
    ...
    const warningPatterns = [
      {
        pattern: /context.*not.*updated/i,
        category: 'MISSING_CONTEXT_UPDATE',
        severity: 'warning',
      },
      {
        pattern: /performance.*threshold/i,
    ...
    const nonBlockingPatterns = [
      {
        pattern: /notification.*fail/i,
        category: 'NOTIFICATION_FAILURE',
        severity: 'non-blocking',
      },
      {
        pattern: /documentation.*generation/i,
        category: 'DOCUMENTATION_FAILURE',
        severity: 'non-blocking',
      },
      {
        pattern: /metrics.*update/i,
        category: 'METRICS_FAILURE',
        severity: 'non-blocking',
      },
      {
        pattern: /cache.*error/i,
  ...
  generateRemediation(classification, recoveryResult) {
    ...
    const remediation = remediations[category] || {
      steps: [
        ...
        'Check hook logs for more information',
      ],
      commands: [],
    };
  ...
  async handleHookError(hookType, error, context = {}) {
    ...
    return {
      ...
      report,
      shouldBlock:
        classification.severity === 'blocking' && !recoveryResult.successful,
    };
  }
}
