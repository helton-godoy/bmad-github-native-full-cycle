...
class ConfigValidator {
    ...
    validatePreCommitConfig(config) {
        ...
        const validOptions = ['linting', 'testing', 'contextValidation', 'gatekeeper'];
    ...
    validateCommitMsgConfig(config) {
        ...
        if (!config.bmadPattern && !config.conventionalCommits) {
            this.warnings.push({
                ...
                recommendation: 'Enable bmadPattern or conventionalCommits for message validation'
            });
        }
    }
    ...
    validatePrePushConfig(config) {
        ...
        if (!config.security) {
            this.warnings.push({
                ...
                recommendation: 'Enable security audits to catch vulnerabilities before pushing'
            });
        }
        ...
        if (!config.fullTests) {
            this.warnings.push({
                ...
                recommendation: 'Enable full tests to ensure code quality before pushing'
            });
        }
    }
    ...
    validatePostCommitConfig(config) {
        ...
        const validOptions = ['metrics', 'documentation', 'notifications', 'contextUpdate'];
    ...
    validatePostMergeConfig(config) {
        ...
        if (!config.workflow) {
            this.warnings.push({
                ...
                recommendation: 'Enable workflow to maintain BMAD automation after merges'
            });
        }
    }
    ...
    validateGitHubActionsSync(config) {
        ...
        const validOptions = ['enabled', 'monitorConsistency', 'reportInconsistencies'];
        ...
        if (!config.enabled) {
            this.warnings.push({
                ...
                recommendation: 'Enable sync to maintain consistency between local and remote validation'
            });
        }
    }
    ...
    validateConsistency(config) {
        ...
        if (config.prePush?.bmadSync && !config.preCommit?.contextValidation) {
            this.warnings.push({
                ...
                recommendation: 'Enable preCommit.contextValidation for consistent BMAD workflow'
            });
        }

        // If GitHub Actions sync is enabled, monitoring should be enabled
        if (config.githubActionsSync?.enabled && !config.githubActionsSync?.monitorConsistency) {
            this.warnings.push({
                type: 'incomplete_sync',
                message: 'GitHub Actions sync is enabled but consistency monitoring is disabled',
                recommendation: 'Enable githubActionsSync.monitorConsistency for full sync benefits'
            });
        }
    }
    ...
    generateReport() {
        ...
        if (result.errors.length > 0) {
            ...
            result.errors.forEach((error, index) => {
                ...
                if (error.remediation) report += `- **Remediation:** ${error.remediation}\n`;
                report += '\n';
            });
        }
    ...
    autoFix() {
        ...
        if (result.errors.some(e => e.type === 'missing_config' || e.type === 'invalid_json')) {
            return {
                success: false,
                message: 'Cannot auto-fix: configuration file is missing or has invalid JSON',
                fixes: []
            };
        }
        ...
        result.errors.forEach(error => {
            if (error.type === 'invalid_type' && error.section && error.option) {
                ...
                fixes.push(`Set ${error.section}.${error.option} to false (was invalid type)`);
            }
        });

        // Remove deprecated options
        result.warnings.forEach(warning => {
            if (warning.type === 'deprecated_option' && warning.section && warning.option) {
                delete config[warning.section][warning.option];
                fixes.push(`Removed deprecated option ${warning.section}.${warning.option}`);
            }
        });
