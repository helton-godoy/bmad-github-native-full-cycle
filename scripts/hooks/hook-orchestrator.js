...
class HookOrchestrator {
  ...
  getMetrics() {
    return {
      ...
      lastExecution:
        this.metrics.executions[this.metrics.executions.length - 1] || null,
    };
  }
  ...
  async executePreCommit(stagedFiles = []) {
    ...
    this.logger.info(
      `Executing pre-commit hook with ${stagedFiles.length} staged files`
    );
    ...
    try {
      ...
      if (this.config.enableGatekeeper) {
        results.gatekeeperIntegration = await this.integrateWithGatekeeper(
          'pre-commit',
          {
            stagedFiles,
            results,
      ...
      const duration = this.endTimer(
        'pre-commit',
        this.allResultsSuccessful(results)
      );
  ...
  async runFastTests() {
    ...
    try {
      ...
      const testResult = await this.testManager.executeTestsWithLock(
        'npm test',
        {
  ...
  async validateContext() {
    ...
    try {
      ...
      const hasCodeChanges = stagedFiles.some((file) =>
        file.match(
          /\.(js|ts|jsx|tsx|py|rb|go|rs|java|c|cpp|h|hpp|css|scss|html|vue|svelte)$/
        )
      );
      ...
      if (hasCodeChanges && !contextStaged) {
        return {
          ...
          contextStaged: false,
          remediation:
            'Update activeContext.md to reflect current changes and stage it',
        };
      }
  ...
  async integrateWithGatekeeper(hookType, context) {
    this.logger.info(
      `Integrating with Enhanced Gatekeeper for ${hookType} hook`
    );
    ...
    try {
      ...
      const validationResult = await this.gatekeeper.validateHookContext(
        hookType,
        gatekeeperContext
      );
      ...
      return {
        status:
          validationResult.gate === 'PASS'
            ? 'passed'
            : validationResult.gate === 'WAIVED'
              ? 'waived'
              : 'failed',
  ...
  async executeCommitMsg(message) {
    ...
    try {
      ...
      const bypassResult = this.checkBypassMechanisms(
        message,
        validationResult
      );
      ...
      if (validationResult.valid && validationResult.format === 'bmad') {
        results.contextValidation = await this.validateCommitContext(
      ...
      if (!success && !bypassResult.bypassed) {
        response.errorMessage =
          validator.generateErrorMessage(validationResult);
        response.remediation = this.generateCommitMessageRemediation(
          validationResult,
          results
        );
      }
  ...
  checkBypassMechanisms(message, _validationResult) {
    ...
    if (this.config.developmentMode) {
      ...
      if (
        ...
        bypassResult.reason =
          'Development mode bypass with WIP/TEMP/DEV prefix';
        ...
        this.logger.warn(
          `Commit message validation bypassed: ${bypassResult.reason}`
        );
        return bypassResult;
      }
      ...
      if (
        ...
        message.toLowerCase().includes('hotfix')
      ) {
        bypassResult.bypassed = true;
    ...
    if (process.env.BMAD_BYPASS_COMMIT_MSG === 'true') {
      bypassResult.bypassed = true;
      bypassResult.reason =
        'Environment variable bypass (BMAD_BYPASS_COMMIT_MSG=true)';
      bypassResult.auditTrail = {
        timestamp: new Date().toISOString(),
        originalMessage: message,
  ...
  async validateCommitContext(parsedMessage) {
    this.logger.info(
      `Validating context consistency for persona: ${parsedMessage.persona}`
    );
    ...
    try {
      ...
      if (!contextExists) {
        return {
          status: 'warning',
          message:
            'activeContext.md not found - consider creating it for better traceability',
          contextExists: false,
      ...
      const stepIdConsistent = this.validateStepIdProgression(
        contextContent,
        parsedMessage.stepId
      );

      return {
        status: personaConsistent && stepIdConsistent ? 'passed' : 'warning',
        message:
          personaConsistent && stepIdConsistent
            ? 'Context validation passed'
  ...
  extractPersonaFromContext(contextContent) {
    ...
    const personaMatch = contextContent.match(
      /(?:persona|role|acting as):\s*([A-Z_]+)/i
    );
    if (personaMatch) {
  ...
  generateCommitMessageRemediation(validationResult, results) {
    ...
    if (!validationResult.valid) {
      remediation.steps.push(
        'Fix commit message format to match BMAD pattern or conventional commits'
      );

      if (validationResult.errors.some((e) => e.includes('BMAD pattern'))) {
        remediation.examples.push(
          '[DEVELOPER] [STEP-001] Implement user authentication'
        );
        remediation.examples.push(
          '[ARCHITECT] [ARCH-042] Design database schema'
        );
        remediation.quickFixes.push(
          'Use format: [PERSONA] [STEP-ID] Description'
        );
      }

      if (validationResult.errors.some((e) => e.includes('persona'))) {
        remediation.quickFixes.push(
          'Valid personas: DEVELOPER, ARCHITECT, PM, QA, DEVOPS, SECURITY, RELEASE, RECOVERY, ORCHESTRATOR'
        );
      }
    }
    ...
    if (results.contextValidation.status === 'failed') {
      ...
      remediation.quickFixes.push(
        'Ensure persona in commit matches current context'
      );
    }
    ...
    if (this.config.developmentMode) {
      remediation.quickFixes.push(
        'Development mode: Use WIP:, TEMP:, or DEV: prefix to bypass validation'
      );
      remediation.quickFixes.push(
        'Emergency: Use "emergency" or "hotfix" in message to bypass'
      );
    }
  ...
  async executePrePush(branch, remote) {
    ...
    try {
      ...
      results.bmadWorkflowSync = await this.synchronizeBMADWorkflow(
        branch,
        remote
      );
  ...
  async runFullTestSuite() {
    ...
    try {
      ...
      const testCommand = pkg.scripts['test:coverage']
      ...
      if (output.includes('{')) {
        ...
        if (jsonMatch) {
          ...
          if (auditData.vulnerabilities) {
            ...
            results.total = Object.values(auditData.vulnerabilities).reduce(
              (sum, count) => sum + count,
              0
            );
          }
  ...
  async synchronizeBMADWorkflow(branch, remote) {
    ...
    try {
      ...
      if (fs.existsSync(contextPath)) {
        ...
        syncResults.currentPersona =
          this.extractPersonaFromContext(contextContent);
        syncResults.workflowPhase = this.extractWorkflowPhase(contextContent);

        // Check persona consistency with recent commits
        syncResults.personaConsistency = await this.validatePersonaConsistency(
          branch,
          syncResults.currentPersona
        );

        // Validate context synchronization
        syncResults.contextSynchronized =
          await this.validateContextSynchronization(contextContent, branch);
      }

      // Check for BMAD orchestrator coordination
      const orchestratorSync = await this.coordinateWithBMADOrchestrator(
        branch,
        remote,
        syncResults
      );
      ...
      return {
        status:
          syncResults.workflowActive &&
          syncResults.personaConsistency &&
          syncResults.contextSynchronized
            ? 'passed'
            : 'warning',
        ...syncResults,
        orchestratorSync,
        message: syncResults.workflowActive
          ? 'BMAD workflow synchronization completed'
          : 'No active BMAD workflow detected',
      };
    } catch (error) {
      this.logger.error(
        `BMAD workflow synchronization failed: ${error.message}`
      );
      return {
        status: 'failed',
        error: error.message,
  ...
  extractWorkflowPhase(contextContent) {
    ...
    const phasePatterns = [
      ...
      {
        pattern: /implementation|coding|development/i,
  ...
  async validatePersonaConsistency(branch, currentPersona) {
    try {
      ...
      return (
        !currentPersona ||
        currentPersona === recentPersona ||
        this.isValidPersonaTransition(recentPersona, currentPersona)
      );
    } catch (error) {
      this.logger.warn(
        `Could not validate persona consistency: ${error.message}`
      );
  ...
  async validateContextSynchronization(contextContent, branch) {
    try {
      ...
      const branchMentioned =
        contextContent.includes(branch) || contextContent.includes('branch');
      ...
      const contextReflectsChanges = recentChanges
        .split('\n')
        .some(
          (file) =>
            contextContent.includes(file) ||
            contextContent.includes(path.basename(file))
        );

      return branchMentioned || hasRecentTimestamp || contextReflectsChanges;
    } catch (error) {
      this.logger.warn(
        `Could not validate context synchronization: ${error.message}`
      );
      return true; // Don't fail on validation errors
    }
  }
  ...
  async coordinateWithBMADOrchestrator(branch, remote, syncResults) {
    try {
      ...
      this.logger.warn(
        `BMAD Orchestrator coordination failed: ${error.message}`
      );
      return {
        available: false,
        coordinated: false,
    ...
    Object.entries(results).forEach(([checkName, result]) => {
      if (result.status === 'failed') {
        switch (checkName) {
          ...
          case 'securityAudit':
            remediation.steps.push(
              'Fix security vulnerabilities before pushing'
            );
          ...
          case 'gatekeeperIntegration':
            remediation.steps.push(
              'Resolve Enhanced Gatekeeper validation issues'
            );
  ...
  async executePostCommit(commitHash) {
    ...
    try {
      ...
      try {
        const notificationResult = await this.notifyBMADOrchestrator(
          commitHash,
          results
        );
      ...
      return {
        ...
        results: {
          metricsUpdate: {
            status: 'warning',
            error: 'Post-commit execution failed before completion',
          },
          documentation: {
            status: 'warning',
            error: 'Post-commit execution failed before completion',
  ...
  async notifyBMADOrchestrator(commitHash, postCommitResults = {}) {
    ...
    try {
      ...
      this.logger.warn(
        `BMAD orchestrator notification warning: ${error.message}`
      );
      return {
        status: 'warning',
        error: error.message,
  ...
  async sendBMADNotification(notificationType, payload) {
    const command =
      'node scripts/bmad/bmad-orchestrator.js --hook-event post-commit';
  ...
  getCommitInfoForNotification(commitHash) {
    ...
    try {
      ...
      this.logger.warn(
        `Could not extract commit info for notification: ${error.message}`
      );
      return defaultInfo;
    }
  }
  ...
  async updateProjectMetrics(commitHash) {
    ...
    try {
      ...
      const metricsPath = path.join(
        process.cwd(),
        '.github/metrics/project-metrics.json'
      );
      let existingMetrics = {
        totalCommits: 0,
        totalLinesAdded: 0,
      ...
      const updatedMetrics = {
        ...
        averageFilesPerCommit:
          (existingMetrics.averageFilesPerCommit *
            existingMetrics.totalCommits +
            filesChanged) /
          (existingMetrics.totalCommits + 1),
  ...
  async regenerateDocumentation(commitHash) {
    this.logger.info(
      `Checking if documentation regeneration is needed for commit ${commitHash}`
    );
    ...
    try {
      ...
      try {
        ...
        return {
          ...
          filesChecked: changedFiles.length,
          sourceFilesChanged: changedFiles.filter((f) =>
            f.match(/\.(js|ts|jsx|tsx|md)$/)
          ),
        };
      } catch (error) {
  ...
  async registerCommitInContext(commitHash) {
    ...
    try {
      // Get commit message
      const commitMessageOutput = execSync(
        `git log --oneline -1 ${commitHash}`,
        {
          encoding: 'utf8',
          stdio: 'pipe',
        }
      ...
      this.logger.error(
        `Failed to register commit in context: ${error.message}`
      );
      throw error;
    }
  }
    ...
    try {
      ...
      try {
        results.mergeAnalysis =
          await this.generateMergeAnalysisReport(mergeType);
      } catch (error) {
      ...
      if (hasFailures) {
        await this.generateRollbackRecommendations(
          recovery,
          results,
          mergeType
        );
        try {
          ...
          this.logger.error(
            `Recovery report generation failed: ${error.message}`
          );
    ...
    try {
      ...
      try {
        ...
        validation.issues.push(
          `Failed to check working tree: ${error.message}`
        );
      }
      ...
      try {
        ...
        validation.hasUnmergedPaths =
          diffCheckOutput.includes('conflict marker');
        if (validation.hasUnmergedPaths) {
          validation.issues.push(
            'Repository has unmerged paths or conflict markers'
          );
        }
      } catch (error) {
  ...
  async generateMergeAnalysisReport(mergeType) {
    ...
    try {
      ...
      const reportPath = path.join(
        process.cwd(),
        '.github/reports/merge-analysis.json'
      );
      ...
      this.logger.error(
        `Failed to generate merge analysis report: ${error.message}`
      );
      throw error;
    }
  }
    ...
    switch (failureType) {
      ...
      case 'report_generation_failed':
        newSteps.push(
          ...
          {
            description:
              'Verify .github/reports directory exists and is writable',
          },
    ...
    try {
      ...
      if (hasRemote) {
        recovery.rollbackRecommendations.push({
          ...
          warning:
            'Force push affects remote repository. Coordinate with team.',
          priority: 'medium',
        });
      }
      ...
      this.logger.error(
        `Failed to generate rollback recommendations: ${error.message}`
      );
  ...
  async generateRecoveryReport(recovery, results, mergeType) {
    ...
    try {
      ...
      const reportPath = path.join(
        process.cwd(),
        '.github/reports/recovery-report.json'
      );
      const reportDir = path.dirname(reportPath);
      if (!fs.existsSync(reportDir)) {
        fs.mkdirSync(reportDir, { recursive: true });
  ...
  async executePreRebase(sourceBranch, targetBranch) {
    ...
    this.logger.info(
      `Executing pre-rebase hook: ${sourceBranch} onto ${targetBranch}`
    );
    ...
    try {
      ...
      results.safetyValidation = await this.validateRebaseSafety(
        sourceBranch,
        targetBranch
      );

      // 2. Check BMAD compatibility
      results.bmadCompatibility =
        await this.validateBMADCompatibility(sourceBranch);

      // 3. Detect potential conflicts
      results.conflictDetection = await this.detectRebaseConflicts(
        sourceBranch,
        targetBranch
      );

      // 4. Analyze commits to be rebased
      results.commitAnalysis = await this.analyzeRebaseCommits(
        sourceBranch,
        targetBranch
      );
  ...
  async validateRebaseSafety(sourceBranch, targetBranch) {
    this.logger.info(
      `Validating rebase safety for ${sourceBranch} onto ${targetBranch}`
    );
    ...
    try {
      ...
      const sourceExists = branches.some(
        (b) => b === sourceBranch || b.endsWith(`/${sourceBranch}`)
      );
      const targetExists = branches.some(
        (b) => b === targetBranch || b.endsWith(`/${targetBranch}`)
      );
  ...
  async validateBMADCompatibility(sourceBranch) {
    ...
    try {
      ...
      const commitsOutput = execSync(
        `git log --oneline ${sourceBranch} --not main develop`,
        {
          encoding: 'utf8',
          stdio: 'pipe',
        }
      ...
      return {
        ...
        message:
          invalidCommits === 0
            ? 'All commits follow BMAD pattern'
            : `${invalidCommits} commits do not follow BMAD pattern`,
  ...
  async detectRebaseConflicts(sourceBranch, targetBranch) {
    this.logger.info(
      `Detecting potential conflicts between ${sourceBranch} and ${targetBranch}`
    );

    try {
      // Get files changed in source branch
      const sourceFilesOutput = execSync(
      ...
      const targetFilesOutput = execSync(
      ...
      const targetFiles = targetFilesOutput
        ? targetFilesOutput.split('\n')
        : [];
      ...
      return {
        ...
        message:
          conflictingFiles.length > 0
            ? `${conflictingFiles.length} files may have conflicts`
  ...
  async executePostCheckout(previousBranch, newBranch, isNewBranch) {
    ...
    this.logger.info(
      `Executing post-checkout hook: ${previousBranch} -> ${newBranch} (new: ${isNewBranch})`
    );
    ...
    try {
      ...
      results.contextRestoration = await this.restoreBranchContext(
        newBranch,
        isNewBranch
      );
  ...
  async executePreReceive(oldCommit, newCommit, refName) {
    ...
    this.logger.info(
      `Executing pre-receive hook: ${refName} (${oldCommit.substring(0, 8)}..${newCommit.substring(0, 8)})`
    );
    ...
    try {
      ...
      results.commitValidation = await this.validatePushedCommits(
        oldCommit,
        newCommit
      );
      ...
      results.authorValidation = await this.validateCommitAuthors(
        oldCommit,
        newCommit
      );

      // 4. Check push size limits
      results.sizeValidation = await this.validatePushSize(
        oldCommit,
        newCommit
      );

      // 5. Detect force pushes
      results.forcePushDetection = await this.detectForcePush(
        oldCommit,
        newCommit,
        refName
      );
  ...
  async validatePushedCommits(oldCommit, newCommit) {
    ...
    try {
      ...
      const commitsOutput = execSync(
        `git rev-list ${oldCommit}..${newCommit}`,
        {
          encoding: 'utf8',
          stdio: 'pipe',
        }
      ...
      for (const hash of commitHashes) {
        try {
          ...
          this.logger.warn(
            `Could not validate commit ${hash}: ${error.message}`
          );
        }
      }
  ...
  async checkBranchProtection(refName) {
    ...
    try {
      ...
      return {
        status: 'passed',
        message: isProtected
          ? 'Branch is protected - strict validation applied'
    ...
    try {
      ...
      for (const hash of commitHashes) {
        try {
          ...
          this.logger.warn(
            `Could not validate author for commit ${hash}: ${error.message}`
          );
          invalidAuthors++;
        }
      }
      ...
      return {
        ...
        message:
          invalidAuthors === 0
            ...
            : `${invalidAuthors} commits have invalid author information`,
  ...
  async validatePushSize(oldCommit, newCommit) {
    ...
    try {
      const commitCountOutput = execSync(
        `git rev-list --count ${oldCommit}..${newCommit}`,
        {
          encoding: 'utf8',
          stdio: 'pipe',
        }
    ...
    try {
      ...
      try {
        ...
        return {
          ...
          message: isProtected
            ? 'Force push to protected branch rejected'
            : 'Force push detected',
          isForcePush: true,
          isNewBranch: false,
          auditTrail,
  ...
  isCacheValid(cachedResult) {
    ...
    return (
      ...
      Date.now() - cachedResult.timestamp < fiveMinutes
    );
  }
  ...
  analyzeValidationConsistency(validationHistory) {
    ...
    const consistencyRate =
      totalValidations > 0 ? consistentCount / totalValidations : 0;
  ...
  synchronizeWithGitHubActions(branch) {
    try {
      ...
      const coordination =
        this.githubActionsSync.validateRemoteCoordination(branch);

      this.logger.info(
        `GitHub Actions sync: ${coordination.recommendation.level} validation recommended`
      );
      ...
      return {
        ...
        recommendation: {
          level: 'comprehensive',
          reason: 'Sync failed - using comprehensive validation',
        },
      };
    }
  }
  ...
  generateConsistencyReport() {
    ...
    const consistencyReport =
      this.githubActionsSync.generateConsistencyReport();
