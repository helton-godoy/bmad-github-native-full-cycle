...
class HooksInstaller {
    ...
    checkPrerequisites() {
        ...
        if (majorVersion < 18) {
            throw new Error(`Node.js >= 18.0.0 required. Current version: ${nodeVersion}`);
        }
        ...
        if (!hasHusky) {
            throw new Error('Husky is not installed. Run "npm install --save-dev husky" first.');
        }
    }
        ...
        hooks.forEach(hook => {
            ...
            if (fs.existsSync(hookPath)) {
                try {
                    ...
                    console.warn(`   ⚠️  Could not set permissions for ${hook}: ${error.message}`);
                }
            }
        });
    }
...
if (require.main === module) {
    ...
    if (command === 'install') {
        ...
        console.log('Usage: node install-hooks.js [install|uninstall] [--force] [--verbose]');
        process.exit(1);
    }
}
