...
class BMADMessageValidator {
  constructor(options = {}) {
    ...
    this.allowConventionalFallback =
      options.allowConventionalFallback !== false; // Default true
  ...
  validateMessage(message) {
    ...
    try {
      ...
      if (this.allowConventionalFallback) {
        const conventionalValidation =
          this._validateConventionalFormat(trimmedMessage);
        ...
        result.errors.push(
          ...conventionalValidation.errors.map(
            (err) => `Conventional format: ${err}`
          )
        );
      } else {
  ...
  generateErrorMessage(validationResult) {
    ...
    errorMessage +=
      '  - PERSONA: Must be uppercase (DEVELOPER, ARCHITECT, PM, QA, DEVOPS, SECURITY, RELEASE, RECOVERY, ORCHESTRATOR)\n';
    errorMessage +=
      '  - STEP-ID: Format like STEP-001, ARCH-042, TEST-123, etc.\n';
  ...
  _validateBMADFormat(message) {
    ...
    if (!match) {
      result.errors.push(
        'Message does not match BMAD pattern: [PERSONA] [STEP-ID] Description'
      );
      return result;
    }
  ...
  _validateConventionalFormat(message) {
    ...
    if (!match) {
      result.errors.push(
        'Message does not match conventional commits pattern: type(scope): description'
      );
      return result;
    }
  ...
  _validateStepId(stepId) {
    ...
    if (!match) {
      result.valid = false;
      result.errors.push(
        'Step ID must follow format: PREFIX-NUMBER (e.g., STEP-001, ARCH-042)'
      );
      return result;
    }
    ...
    if (!validPrefixes.includes(prefix)) {
      result.warnings.push(
        `Step ID prefix '${prefix}' is not standard. Common prefixes: ${validPrefixes.join(', ')}`
      );
    }
    ...
    if (numericValue < 1) {
      ...
      result.warnings.push(
        'Step ID number is very large - consider using smaller numbers'
      );
    }
    ...
    if (number.length < 3 && numericValue < 100) {
      result.warnings.push(
        'Consider padding step ID numbers to 3 digits (e.g., STEP-001)'
      );
    }
  ...
  _validateDescription(description) {
    ...
    if (trimmedDescription.length > 100) {
      result.warnings.push(
        'Description is very long - consider shortening for better readability'
      );
    }
    ...
    for (const phrase of vaguePhrases) {
      if (
        lowerDescription === phrase ||
        lowerDescription.startsWith(phrase + ' ')
      ) {
        if (trimmedDescription.length < 20) {
          result.warnings.push(
            `Description "${phrase}" is too generic - be more specific about what was ${phrase}ed`
          );
        }
      }
    }
    ...
    if (trimmedDescription.endsWith('.')) {
      result.warnings.push(
        'Commit descriptions typically do not end with a period'
      );
    }
  ...
  getValidationSummary(validationResult) {
    if (validationResult.valid) {
      const format =
        validationResult.format === 'bmad' ? 'BMAD' : 'Conventional Commits';
      const warnings =
