...
class GitHubActionsSync {
    constructor(config = {}) {
        this.config = {
            ...
            syncReportFile: path.join(process.cwd(), '.git', 'github-actions-sync-report.json'),
            ...config
        };
    ...
    synchronizeConfiguration() {
        try {
            ...
            const inconsistencies = this.compareConfigurations(localConfig, githubWorkflows);
    ...
    getGitHubWorkflows() {
        ...
        try {
            ...
            const workflowFiles = fs.readdirSync(this.config.workflowsDir)
    ...
    extractValidationSteps(workflow) {
        ...
        for (const jobName in workflow.jobs) {
            ...
            for (const step of job.steps) {
                ...
                if (stepName.includes('test') || stepRun.includes('npm test') || stepRun.includes('npm run test')) {
                    validationSteps.testing = true;
                }
                if (stepName.includes('build') || stepRun.includes('npm run build')) {
                    validationSteps.build = true;
                }
                if (stepName.includes('audit') || stepName.includes('security') || stepRun.includes('npm audit')) {
    ...
    compareConfigurations(localConfig, githubWorkflows) {
        ...
        for (const workflowName in githubWorkflows) {
            ...
            aggregatedGitHub.security = aggregatedGitHub.security || workflow.security;
        }
    ...
    generateConsistencyReport() {
        ...
        const consistentCount = this.consistencyHistory.filter(e => e.consistent).length;
    ...
    validateRemoteCoordination(branch) {
        ...
        const willRunRemote = protectedBranches.includes(branch) || branch.startsWith('feature/');

        return {
            willRunRemote,
            syncReport,
            recommendation: this.getValidationRecommendation(willRunRemote, syncReport)
        };
    }
    ...
    getValidationRecommendation(willRunRemote, syncReport) {
        if (!willRunRemote) {
            return {
                level: 'comprehensive',
                reason: 'GitHub Actions will not run for this branch - comprehensive local validation required'
            };
        }
        ...
        if (!syncReport.consistent) {
            return {
                level: 'comprehensive',
                reason: 'Configuration inconsistencies detected - comprehensive local validation recommended'
            };
        }

        return {
            level: 'standard',
            reason: 'GitHub Actions will run and configurations are consistent - standard validation sufficient'
        };
    }
}
