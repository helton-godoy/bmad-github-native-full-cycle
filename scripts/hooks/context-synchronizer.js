...
class ContextSynchronizer {
  ...
  async updateActiveContext(commitInfo) {
    this.logger.info(
      `Updating activeContext.md for commit: ${commitInfo.hash || 'staged'}`
    );
    ...
    try {
      ...
      const contextUpdate = {
        ...
        persona:
          commitInfo.persona ||
          this.extractPersonaFromMessage(commitInfo.message),
        stepId:
          commitInfo.stepId ||
          this.extractStepIdFromMessage(commitInfo.message),
      ...
      const updatedContext = this.generateUpdatedContext(
        existingContext,
        contextUpdate
      );

      // Write updated context using ContextManager for atomic operations
      const writeHash = await this.contextManager.write(
        this.config.contextFile,
        updatedContext
      );

      this.logger.info(
        `Context updated successfully. Hash: ${writeHash.substring(0, 8)}`
      );
  ...
  async syncPersonaState(persona, stepId) {
    ...
    try {
      ...
      syncResults.transitionValidation =
        await this.validatePersonaTransition(persona);

      // 3. Sync with activeContext.md
      syncResults.contextSync = await this.syncContextWithPersona(
        persona,
        stepId
      );
      ...
      const allValid = Object.values(syncResults).every(
        (result) => result.valid !== false && result.success !== false
      );
      ...
      return {
        ...
        results: syncResults,
        message: allValid
          ? 'Persona state synchronized successfully'
          : 'Persona state synchronization completed with issues',
      };
    } catch (error) {
      this.logger.error(
        `Persona state synchronization failed: ${error.message}`
      );
      return {
        success: false,
        error: error.message,
  ...
  async validateContextConsistency() {
    ...
    try {
      ...
      validationResults.personaConsistency =
        await this.validatePersonaConsistency(validationResults.contextContent);

      // 3. Validate workflow consistency
      validationResults.workflowConsistency =
        await this.validateWorkflowConsistency(
          validationResults.contextContent
        );

      // 4. Validate BMAD handover consistency
      validationResults.handoverConsistency =
        await this.validateHandoverConsistency(
          validationResults.contextContent
        );

      // 5. Validate git state consistency
      validationResults.gitStateConsistency =
        await this.validateGitStateConsistency(
          validationResults.contextContent
        );
      ...
      return {
        ...
        recommendations:
          this.generateConsistencyRecommendations(validationResults),
      };
    } catch (error) {
      this.logger.error(
        `Context consistency validation failed: ${error.message}`
      );
      return {
        consistent: false,
        error: error.message,
  ...
  determineWorkflowPhase(commitInfo) {
    const persona =
      commitInfo.persona || this.extractPersonaFromMessage(commitInfo.message);
  ...
  generateCommitSummary(commitInfo) {
    const persona =
      commitInfo.persona || this.extractPersonaFromMessage(commitInfo.message);
    const stepId =
      commitInfo.stepId || this.extractStepIdFromMessage(commitInfo.message);
  ...
  generateUpdatedContext(existingContext, contextUpdate) {
    ...
    if (existingContext) {
      ...
      if (previousEntries.length > 0) {
        ...
        const entriesToKeep = previousEntries.slice(
          0,
          this.config.maxContextHistory - 1
        );
  ...
  async validatePersonaTransition(newPersona) {
    ...
    try {
      ...
      if (!currentPersona) {
        ...
        result.warnings.push(
          'No current persona found in context - transition validation skipped'
        );
        return result;
      }
    ...
    try {
      ...
      return {
        ...
        message: result.success
          ? 'Context synchronized with persona'
          : 'Context sync failed',
        error: result.error,
      };
    } catch (error) {
  ...
  async syncBMADHandover(persona, _stepId) {
    try {
      ...
      return {
        ...
        message: consistent
          ? 'Handover state is consistent'
          : 'Handover state may be inconsistent',
      };
    } catch (error) {
      return {
        success: false,
    ...
    try {
      ...
      return {
        ...
        reason: consistent
          ...
          : 'Persona inconsistency detected',
      };
    } catch (error) {
      return {
        consistent: false,
  ...
  async validateWorkflowConsistency(contextContent) {
    try {
      ...
      const expectedPhase = this.determineWorkflowPhase({
        persona: contextPersona,
      });
      const consistent =
        contextPhase === expectedPhase || contextPhase === 'unknown';
      ...
      return {
        ...
        contextPersona,
        reason: consistent
          ? 'Workflow phase is consistent with persona'
          : 'Workflow phase inconsistency detected',
      };
    } catch (error) {
      return {
        consistent: false,
  ...
  async validateHandoverConsistency(contextContent) {
    try {
      ...
      const consistent =
        !handoverPersona ||
        !contextPersona ||
        handoverPersona === contextPersona;
      ...
      return {
        ...
        contextPersona,
        reason: consistent
          ? 'Handover and context personas are consistent'
          : 'Handover and context persona mismatch',
      };
    } catch (error) {
      return {
        consistent: false,
  ...
  async validateGitStateConsistency(contextContent) {
    try {
      ...
      const contextReflectsChanges = recentFiles.some(
        (file) =>
      ...
      const timestampPattern =
        /\*\*Last Updated:\*\*\s*(\d{4}-\d{2}-\d{2}T[\d:.-]+Z?)/;
      ...
      return {
        ...
        recentFiles,
        reason: consistent
          ? 'Context is consistent with git state'
          : 'Context may be outdated relative to git state',
      };
    } catch (error) {
      return {
        consistent: false,
  ...
  generateConsistencyRecommendations(validationResults) {
    ...
    if (!validationResults.contextFileExists) {
      recommendations.push(
        'Create activeContext.md to track current work and maintain BMAD workflow state'
      );
    }

    if (!validationResults.personaConsistency.consistent) {
      recommendations.push(
        'Update activeContext.md to reflect the current persona and ensure consistency with recent commits'
      );
    }

    if (!validationResults.workflowConsistency.consistent) {
      recommendations.push(
        'Align workflow phase in activeContext.md with the current persona and work being performed'
      );
    }

    if (!validationResults.handoverConsistency.consistent) {
      recommendations.push(
        'Synchronize BMAD handover state with activeContext.md to maintain consistency across components'
      );
    }

    if (!validationResults.gitStateConsistency.consistent) {
      recommendations.push(
        'Update activeContext.md to reflect recent changes and maintain currency with git state'
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        'Context consistency is good - continue maintaining regular updates'
      );
    }
