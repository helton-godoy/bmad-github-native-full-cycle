...
class HookSetupCLI {
    ...
    displayConfigReport(report) {
        ...
        this.printInfo(`Husky: ${report.configuration.huskyVersion || 'Not installed'}`);
        ...
        if (report.hooks.length === 0) {
            ...
            report.hooks.forEach(hook => {
                const status = hook.exists
                    ? (hook.executable ? '✅ Installed' : '⚠️  Not executable')
                    : '❌ Missing';
                ...
                if (hook.exists) {
                    ...
                    console.log(`    ${colors.dim}Modified: ${hook.lastModified}${colors.reset}`);
                }
            });
        }
    ...
    async run() {
        try {
            ...
            this.printInfo('This wizard will help you set up git hooks for the BMAD workflow');
            console.log('');
