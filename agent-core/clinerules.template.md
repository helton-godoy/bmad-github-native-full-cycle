...
2.  **Update Memory:** You MUST update `activeContext.md` with your progress before every `git commit`.
    - _Why?_ If you don't write it down, the next agent (or you in the future) won't know it happened.
...
1.  **Read:** Check `activeContext.md` + `docs/planning/` + GitHub Issue.
    - _AgentDoc:_ If modifying complex logic, check `docs/architecture/SYSTEM_MAP.md` for invariants.
    - _Vector Memory:_ For deep context, run `npm run bmad:search "your query"`.
2.  **Plan:** Create/Update documentation in `docs/architecture/` before coding.
3.  **Act:** Write code in `src/` + Tests in `tests/`.
    - _AgentDoc:_ Add `@ai-context`, `@ai-invariant`, and `@ai-connection` comments to your code.
4.  **Verify:** Run `npm test` and `npm run bmad:gatekeeper`.
    - _AgentDoc:_ Run `npm run bmad:doc` to update the system map.
